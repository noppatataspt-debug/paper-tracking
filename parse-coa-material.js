...
// COA บางใบใช้หัวจดหมายภาษาไทย บางใบภาษาอังกฤษ → ชื่อเดียวกันแต่คนละ string
// เช่น "PERMAFLEX CO., LTD." กับ "บริษัท เปอร์มาเฟล็กซ์ จำกัด"
// map ทุกแบบเข้าชื่อสั้นชื่อเดียว เพื่อไม่ให้ dropdown แตกเป็นหลายราย
// ⚠️ ค่าที่ใช้ตรงนี้ต้องตรงกับที่เก็บใน Supabase (ดู normalize_supplier_name() ใน SQL)
const SUPPLIER_ALIASES = [
  { canonical: 'Permaflex',   match: [/permaflex/i, /เปอร์มาเฟล็ก/, /เปอร์มาเฟลก/] },
  { canonical: 'Bestak',      match: [/bestak/i, /เบสแท/] },
  { canonical: 'Surint Omya', match: [/omya/i, /โอมย่า/, /สุรินทร์\s*โอ/] },
];
...
export function normalizeSupplier(name) {
  ...
  for (const a of SUPPLIER_ALIASES) {
    if (a.match.some(re => re.test(s))) return a.canonical;
  }
  return s; // ไม่รู้จัก → คืนค่าเดิม (แค่ trim ช่องว่างเกิน)
}
...
โครงสร้าง JSON ที่ต้องคืน:
{
  ...
  "supplier": "ชื่อแบรนด์/บริษัทผู้ผลิต-ผู้ขาย แบบสั้นภาษาอังกฤษ (ไม่ใช่ Asia Papertube ซึ่งเป็นผู้รับ) — ตัดคำว่า บริษัท/จำกัด/CO.,LTD./LIMITED ออก และถ้าหัวจดหมายเป็นภาษาไทยให้แปลงเป็นชื่ออังกฤษ เช่น 'บริษัท เปอร์มาเฟล็กซ์ จำกัด' → 'Permaflex', 'BESTAK CHEMICAL LTD.' → 'Bestak', 'Surint Omya Chemical' → 'Surint Omya'",
...
export default async function handler(req, res) {
  ...
  // normalize ชื่อ supplier (กันชื่อไทย/อังกฤษของรายเดียวกันแตกเป็น 2 ราย)
  // เก็บค่าดิบไว้ใน supplier_raw เผื่อตรวจย้อนหลัง
  if (parsed.supplier) {
    const canon = normalizeSupplier(parsed.supplier);
    if (canon !== parsed.supplier) parsed.supplier_raw = parsed.supplier;
    parsed.supplier = canon;
  }
